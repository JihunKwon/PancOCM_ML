...
Distance is calculated by using Mahalanobis distance.
'''
...
for y in range(0, np.size(tole_list)):
    ...
    #for fidx in range(6, 8):
        ...
        ### Original code. |M - U'| > m*SD ###
        # set threshold
        thr0[:] = m0 * np.square(sd0[:])
        thr1[:] = m1 * np.square(sd1[:])
        thr2[:] = m2 * np.square(sd2[:])
            ...
            for p in range(0, t_sub_removed):
                ...
                for depth in range(0, s_new):
                    ...
                    if flag0 < 1:  # OCM0
                        ...
                        if np.square(ocm0_low[depth, bh * t_sub_removed + p] - median0_base[depth]) > thr0[depth]:
                            out0[bh] = out0[bh] + 1
                            flag0 = 1
        ...
        for bh in range(0, num_bh):
            for p in range(0, t_sub):
                ...
                for depth in range(0, s_new):
                    # if not detected yet
                    if flag1 < 1:  # OCM1
                        if np.square(ocm1_low[depth, bh * t_sub + p] - median1_base[depth]) > thr1[depth]:
                            out1[bh] = out1[bh] + 1
                            flag1 = 1
                    if flag2 < 1:  # OCM2
                        if np.square(ocm2_low[depth, bh * t_sub + p] - median2_base[depth]) > thr2[depth]:
                            out2[bh] = out2[bh] + 1
                            flag2 = 1
