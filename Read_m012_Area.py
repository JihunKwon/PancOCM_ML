...
tole_area_list = [0.0001]
train_list = [2, 3, 4]

for x in range(0, np.size(tole_area_list)):
    for y in range(0, np.size(train_list)):
        for fidx in range(0, np.size(rep_list)):
            num_train = train_list[x]
            tole = tole_area_list[y]
            ...
            if fidx % 2 == 0:
                ...
                fname = 'm012_' + str(Sub_run_name) + '_bin' + str(bin) + '_scale' + str(scale) + '_train' + str(num_train) + '_area_pr.pkl'
                #fname = 'm012_s1r1_bin500_scale50_train2.pkl'
                print('Reading file: ', fname)
                with open(fname, 'rb') as f:
                    median0_base, median1_base, median2_base, sd0, sd1, sd2, out_area0, out_area1, out_area2 = pickle.load(f)
                ...
                for m in range(0, bin):
                    ...
                    if (out_area0[m] < out_area0[0] * tole) and (flag0_m == 0):
                        m0 = m / scale
                        flag0_m = 1
                    if (out_area1[m] < out_area1[0] * tole) and (flag1_m == 0):
                        m1 = m / scale
                        flag1_m = 1
                    if (out_area2[m] < out_area2[0] * tole) and (flag2_m == 0):
                        m2 = m / scale
                        flag2_m = 1

                print('m0:', m0, 'm1:', m1, 'm2:', m2)
                print('count0:', out_area0[int(m0*scale)], 'count1:', out_area1[int(m1*scale)], 'count2:', out_area2[int(m2*scale)])
