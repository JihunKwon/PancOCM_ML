...
for length_idx in range(0, len(length_list)):
    ...
    for fidx in range(0, np.size(rep_list)):
        ...
        for p in range(0, c0_new_removed):
            if p % interval is 0:
                ocm0_ave[:, count0] = np.median(ocm0[:, count0*interval:(count0+1)*interval], axis=1)
                count0+=1
        for p in range(0, c0_new):
            if p % interval is 0:
                ocm1_ave[:, count12] = np.median(ocm1[:, count12*interval:(count12+1)*interval], axis=1)
                ocm2_ave[:, count12] = np.median(ocm2[:, count12*interval:(count12+1)*interval], axis=1)
                count12+=1
        # check zero array
        ...
        # Make sure low length can be devided by interval
        low_len0 = ocm0_ave.shape[1]
        low_len1 = ocm1_ave.shape[1]
        low_len2 = ocm2_ave.shape[1]
        print('initial low_len0', ocm0_ave.shape[1])
        print('initial low_len1', ocm1_ave.shape[1])
        print('initial low_len2', ocm2_ave.shape[1])
        while low_len0 % interval is not 0:
            low_len0 -= 1
        while low_len1 % interval is not 0:
            low_len1 -= 1
        while low_len2 % interval is not 0:
            low_len2 -= 1
        ocm0_ave = ocm0_ave[:, :low_len0]
        ocm1_ave = ocm1_ave[:, :low_len1]
        ocm2_ave = ocm2_ave[:, :low_len2]
        print('updated low_len0', ocm0_ave.shape[1])
        print('updated low_len1', ocm1_ave.shape[1])
        print('updated low_len2', ocm2_ave.shape[1])
        ...
        t_sub = int(c0_new / num_bh)
        t_sub_removed = int(c0_new_removed / num_bh)
        print('t_sub_removed', t_sub_removed)
        print('t_sub', t_sub)
        ...
        for bh in range(0, num_bh):
            for depth in range(0, s_new):
                ...
                median0[depth, bh] = statistics.median(ocm0_ave[depth, bh * t_sub_removed:(bh + 1) * t_sub_removed])
                median1[depth, bh] = statistics.median(ocm1_ave[depth, bh * t_sub:(bh + 1) * t_sub])
                median2[depth, bh] = statistics.median(ocm2_ave[depth, bh * t_sub:(bh + 1) * t_sub])
        ...
        for p in range(0, c0_new_removed):
            ...
            for depth in range(0, s_new):
                ...
                ocm0_filt[depth, p] = ocm0_ave[depth, p] - median0_low[depth, bh]
            tr0 = ocm0_filt[:, p]
        ...
        for p in range(0, c0_new):
            ...
            for depth in range(0, s_new):
                # filter the signal (subtract median from each trace of corresponding bh)
                ocm1_filt[depth, p] = ocm1_ave[depth, p] - median1_low[depth, bh]
                ocm2_filt[depth, p] = ocm2_ave[depth, p] - median2_low[depth, bh]
            tr1 = ocm1_filt[:, p]
            tr2 = ocm2_filt[:, p]
        ...
        if fidx % 2 == 0:
            ...
            for depth in range(0, s_new):
                ...
                A0_pre_medi[depth] = statistics.mean(A0_pre[depth, :])
                A1_pre_medi[depth] = statistics.mean(A1_pre[depth, :])
                A2_pre_medi[depth] = statistics.mean(A2_pre[depth, :])
                A0_post_medi[depth] = statistics.mean(A0_post[depth, :])
                A1_post_medi[depth] = statistics.mean(A1_post[depth, :])
                A2_post_medi[depth] = statistics.mean(A2_post[depth, :])
        ...
        if fidx % 2 == 1:
            ...
            a1 = ax1.plot(d, A0_pre_medi[:], linewidth=2.0, color='k', label='mean')
            ...
            a2 = ax2.plot(d, A1_pre_medi[:], linewidth=2.0, color='k', label='mean')
            plt.axhline(y=chi2_interval_max_0000000001, color='k', linestyle='--', linewidth=2, label='0.000001% threshold')
            ...
            a3 = ax3.plot(d, A2_pre_medi[:], linewidth=2.0, color='k', label='mean')
            plt.axhline(y=chi2_interval_max_0000000001, color='k', linestyle='--', linewidth=2, label='0.000001% threshold')
            ...
            if fidx is not 11:  # The data in OCM0 for s3r2 state 2 is strange. We don't use this.
                ...
                a4 = ax4.plot(d, A0_post_medi[:], linewidth=2.0, color='k', label='mean')
                plt.axhline(y=chi2_interval_max_0000000001, color='k', linestyle='--', linewidth=2, label='0.000001% threshold')
            ...
            a5 = ax5.plot(d, A1_post_medi[:], linewidth=2.0, color='k', label='mean')
            plt.axhline(y=chi2_interval_max_0000000001, color='k', linestyle='--', linewidth=2, label='0.000001% threshold')
            ...
            a6 = ax6.plot(d, A2_post_medi[:], linewidth=2.0, color='k', label='mean')
            plt.axhline(y=chi2_interval_max_0000000001, color='k', linestyle='--', linewidth=2, label='0.000001% threshold')
